 ...
 * @param string input
 */
ShortcodeParser.prototype.parse = async function (input, options) {
  var self = this;
  
  var tokenizer = Tokenizer(input, options);

  await tokenizer.tokenize(ShortcodeParser.parsers.tagOpen, this);
...
ShortcodeParser.parsers.error = function (t, ctx) {
  if (ctx && ctx.name) {
    ...
    if (ctx.errorUnclosed)
      t.toBuffer(
        "!" + this.options.closePattern.replace(/\\/g, "") + ctx.content
      );
    else if (ctx.content !== "")
      t.toBuffer(
        "!" +
          this.options.closePattern.replace(/\\/g, "") +
          ctx.content +
          this.options.openPattern.replace(/\\/g, "") +
          "/!" +
          ctx.name +
          "!" +
          this.options.closePattern.replace(/\\/g, "")
      );
    else t.toBuffer("!/" + this.options.closePattern.replace(/\\/g, ""));
  } else {
    t.toBuffer(
      this.options.openPattern.replace(/\\/g, "") +
        "^!" +
...
ShortcodeParser.parsers.tagBody = function (t, ctx) {
  ...
  if (t.isEnd()) return ShortcodeParser.parsers.error;

  //Attributes?
  if (
    t.match(
      ".*?(?=(\\/)?" + this.options.closePattern + "|$)",
      false,
      false,
      false
    )
...
ShortcodeParser.parsers.params = function (t, ctx) {
  var addParam = function () {
    if (ctx.argKey) ctx.args[ctx.argKey] = t.clearBuffer();
    else if (t.getBuffer().trim() !== "") ctx.args.push(t.clearBuffer());

    delete ctx.argKey;
  };

  if (
    !t.match("^\\/" + this.options.closePattern, false, false, false) &&
    !t.match("^" + this.options.closePattern, false, false, false) &&
    !t.isEnd()
  ) {
    //Escape
    if (t.match("^\\\\")) {
      ...
      if (ctx.argKey) t.toBuffer("=");
      else ctx.argKey = t.clearBuffer();
    } else {
      t.next(true, true);
    }
...
ShortcodeParser.parsers.content = function (t, ctx) {
  ...
  if (t.match(this.options.openPattern, true)) {
    //Close
    if (t.match("^\\/", false, false, false)) {
      if (t.match("^\\/" + ctx.name + this.options.closePattern, true)) {
        ctx.content += t.getBuffer();
        t.skip(-this.options.closePattern.replace(/\\/g, "").length);

        return ShortcodeParser.parsers.close;
      } else {
        t.skip(-this.options.openPattern.replace(/\\/g, "").length);
        t.eat(this.options.openPattern.replace(/\\/g, "").length + 1);
...
ShortcodeParser.parsers.process = async function (t, ctx) {
  //if not shortchode
  if (!ctx || !ctx.name || ctx.name === "") return false;
  ...
  const shortcodeOutput = await this.shortcodes[ctx.name].call(
    ctx,
    ctx.args,
    ctx.content,
    t.options
